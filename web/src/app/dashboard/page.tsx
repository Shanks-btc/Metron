...
export const metadata: Metadata = {
  title: "Dashboard - Metron",
  description: "Live negotiation activity from the real Metron backend — metadata only, never the paid intelligence data itself.",
};
      ...
      <main className="w-full max-w-full">
        <section className="relative w-full overflow-hidden px-4 py-12 sm:px-6 sm:py-14 lg:px-8 lg:py-16">
          <div className="mx-auto flex w-full max-w-3xl flex-col items-center text-center">
            ...
            <h1 className="mt-4 w-full text-balance break-words font-display text-3xl font-bold text-ink-heading sm:text-4xl">
              Every negotiation, straight from the METRON backend.
            </h1>
            <p className="mt-4 w-full max-w-xl text-balance break-words text-sm leading-relaxed text-ink-body sm:text-base">
              Every negotiation Metron settles is logged here. Settlement
              runs via x402 on Base mainnet: no invoices, subscriptions, or manual
        ...
        <section className="relative w-full overflow-hidden border-t border-subtle px-4 py-12 sm:px-6 sm:py-14 lg:px-8 lg:py-16">
          <div className="mx-auto flex w-full max-w-3xl flex-col items-center text-center">
            ...
            <p className="mt-3 w-full max-w-xl text-balance break-words text-sm leading-relaxed text-ink-body sm:text-base">
              Every accept, counter, and reject from the same negotiation layer that settles on Base.
            </p>
          </div>
        ...
        {/* Deliberately smaller and last among the data sections - a direct
            on-chain balance read, not derived from Postgres like the
            sections above. Equal visual weight to the sections above would
            overstate it as a revenue/accounting figure rather than what it
            actually is: the seller wallet's real, current Base USDC
            balance, independently verifiable on Basescan by anyone. */}
        <section className="relative w-full overflow-hidden border-t border-subtle px-4 py-8 sm:px-6 sm:py-10 lg:px-8">
          <div className="mx-auto flex w-full max-w-2xl flex-col items-center text-center">
            <span className="inline-flex items-center rounded-full border border-subtle px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wide text-ink-label">
              On-chain
            </span>
            <h2 className="mt-3 w-full text-balance break-words font-display text-lg font-semibold text-ink-body sm:text-xl">
              Seller balance — real USDC on Base mainnet.
            </h2>
            <p className="mt-2 w-full max-w-xl text-balance break-words text-xs leading-relaxed text-ink-label sm:text-sm">
              A direct on-chain balance read of the seller wallet&apos;s USDC
              holdings on Base — independently verifiable by anyone via
              Basescan, not a number this API is trusted to report honestly.
            </p>
          </div>
          <div className="mx-auto mt-6 flex w-full max-w-[15rem] flex-col items-center rounded-xl border border-subtle px-5 py-4 text-center">
            <p className="text-[10px] font-medium uppercase tracking-wide text-ink-label">
              Seller USDC balance
            </p>
            <p className="mt-1 font-display text-xl font-semibold text-ink-body">
              {revenue ? (
                <AnimatedNumber target={Number(revenue.usdcBalance)} prefix="$" decimals={6} />
              ) : (
                "—"
              )}
            </p>
            <p className="mt-1 text-[11px] text-ink-label">
              {revenue ? "on Base mainnet" : "backend unreachable"}
            </p>
          </div>
        </section>
