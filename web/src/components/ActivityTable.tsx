          ...
          {rows.map((row, i) => (
            ...
            >
              ...
              <td className="whitespace-nowrap px-4 py-3">
                {/* payerAddress is populated once a quote's GET /pay/:id
                    settles for real (see server.ts's resourceServer.
                    onAfterSettle) - the real payer from the x402
                    facilitator's settle response on Base mainnet. */}
                {row.payerAddress ? (
                  <a
                    href={`https://basescan.org/address/${row.payerAddress}`}
                    target="_blank"
